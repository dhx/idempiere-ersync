...
/**
 *	Import Order from I_Web_Order (adapted from I_Order)
 *  @author Oscar Gomez
 ...
 */
public class ImportWebOrder extends SvrProcess
{
	...
	protected String doIt() throws java.lang.Exception
	{
		...
		{
			sql = new StringBuilder ("DELETE I_Web_Order ")
		...
		//	Document Type - PO - SO
		sql = new StringBuilder ("UPDATE I_Web_Order o ")	//	PO Document Type Name
		...
		sql = new StringBuilder ("UPDATE I_Web_Order o ")	//	SO Document Type Name
			  .append("SET C_DocType_ID=(SELECT C_DocType_ID FROM C_DocType d WHERE d.Name=o.DocTypeName")
		...
		sql = new StringBuilder ("UPDATE I_Web_Order ")	//	Error Invalid Doc Type Name
		...
		//	DocType Default
		sql = new StringBuilder ("UPDATE I_Web_Order o ")	//	Default PO
		...
		sql = new StringBuilder ("UPDATE I_Web_Order o ")	//	Default SO
			  .append("SET C_DocType_ID=(SELECT MAX(C_DocType_ID) FROM C_DocType d WHERE d.IsDefault='Y'")
			  .append(" AND d.DocBaseType='SOO' AND o.AD_Client_ID=d.AD_Client_ID) ")
		...
		sql = new StringBuilder ("UPDATE I_Web_Order ")	// No DocType
		...
		//	Set IsSOTrx
		sql = new StringBuilder ("UPDATE I_Web_Order o SET IsSOTrx='Y' ")
		...
		sql = new StringBuilder ("UPDATE I_Web_Order o SET IsSOTrx='N' ")
		...
		**/
		...
		sql = new StringBuilder ("SELECT * FROM I_Web_Order ")
			  ...
			  .append(" ORDER BY I_Web_Order_ID ASC");
		...
		try
		{
			...
			while (rs.next ())
			{
				X_I_Web_Order imp = new X_I_Web_Order (getCtx (), rs, get_TrxName());
		...
		sql = new StringBuilder ("UPDATE I_Web_Order o ")
		  ...
		  .append("WHERE I_IsImported<>'Y'").append (clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		if (log.isLoggable(Level.FINE)) log.fine("Set FreightAmt from other line=" + no);
		...
		sql = new StringBuilder ("SELECT * FROM I_Web_Order ")
			...
			.append(" ORDER BY C_BPartner_ID, BillTo_ID, C_BPartner_Location_ID, I_Web_Order_ID");
		try
		{
			pstmt = DB.prepareStatement (sql.toString(), get_TrxName());
			rs = pstmt.executeQuery ();
			...
			while (rs.next ())
			{
					...
					|| !oldDocumentNo.equals(cmpDocumentNo))
				{
					...
					if (imp.getFreightAmt() != null && imp.getFreightAmt().signum() != 0) {
						order.setFreightAmt(imp.getFreightAmt());
						order.setFreightCostRule("F"); // Set the freight cost rule to fixed
					}
